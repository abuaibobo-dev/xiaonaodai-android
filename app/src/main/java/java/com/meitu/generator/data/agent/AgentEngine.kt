...
请基于以上结果，用简洁友好的中文回复用户。"""
    ...
    private suspend fun buildSystemPrompt(memoryPrompt: String, currentModel: String, webSearchEnabled: Boolean, hasImage: Boolean): String {
        ...
        sb.appendLine("1. 用户说\"帮我做XX\" → 立即开始拆解任务、生成代码、执行，不反问不拖延")
        sb.appendLine("2. 用户说\"想做个XX\" → 先问2个关键问题，再给方案，确认后执行")
        sb.appendLine("3. 用户说\"帮我看看XX\" → 分析问题，给出具体建议，不要只说\"可以的\"")
        sb.appendLine("4. 用户说\"你好\" → 简单问候，但准备好随时进入工作状态")
        sb.appendLine("5. 用户说\"XX是什么\" → 知识性回答，结束后问一句\"需要我帮你实现吗\"")
        ...
        sb.appendLine("2. 模糊需求先问2-3个关键问题，问完直接给建议方案，不让用户做选择题")
        sb.appendLine("3. 明确需求直接干，不反复确认")
        sb.appendLine("4. 记住用户历史偏好，下次自动应用")
        sb.appendLine("5. 错误提示说人话，不说技术术语")
        sb.appendLine("6. 用中文回复，简洁直接，不啰嗦")
        sb.appendLine("7. 每次回答前，必须先在 \`\`\`thinking\`\`\` 标签里写出你的推理过程，再输出最终答案。即使用户没要求，也要这么做")
        sb.appendLine("8. 任何时候用户提出需求，先输出一个完整的执行计划，不要直接动手。格式如下：")
        sb.appendLine("   \`\`\`plan")
        sb.appendLine("   目标：[一句话概括用户要什么]")
        sb.appendLine("   步骤：")
        sb.appendLine("   1. [步骤1]")
        sb.appendLine("   2. [步骤2]")
        sb.appendLine("   确认：[是否还需要用户提供信息]")
        sb.appendLine("   \`\`\`")
        sb.appendLine()
