                ...
                try {
                    ...
                    when (event) {
                        ...
                        "conversation.message.delta" -> {
                            ...
                            val msgType = msg?.get("type")?.asString ?: json.get("type")?.asString ?: ""
                            if (content.isNotEmpty()) {
                                fullText.append(content)
                                emit(StreamEvent.Delta(content))
                            } else if (reasoning.isNotEmpty() && msgType == "answer") {
                                // Coze API 有时把回复内容放在 reasoning_content 字段
                                fullText.append(reasoning)
                                emit(StreamEvent.Delta(reasoning))
                        ...
                        "conversation.message.completed" -> {
                            ...
                            val content = msg?.get("content")?.asString ?: json.get("content")?.asString ?: ""
                            val reasoning = msg?.get("reasoning_content")?.asString ?: json.get("reasoning_content")?.asString ?: ""
                            if (type == "answer" && fullText.isEmpty()) {
